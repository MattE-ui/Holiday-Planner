...
export function AtelierLanding({
  ...
  return (
    <div className="relative min-h-[100svh] w-full overflow-hidden bg-[hsl(198_36%_9%)] text-white">
      ...
      <div className="atelier-rise absolute bottom-12 left-10 z-[35] hidden max-w-[560px] md:block">
        <div className="mb-4">
          <StatusChip label={status} />
        </div>
        <h1 className="font-display text-[72px] font-semibold leading-[0.98] tracking-[-0.025em] [text-shadow:0_2px_30px_rgba(0,0,0,0.34)]">
          {tripName}
        </h1>
        <div className="mt-[18px] flex items-center gap-[18px] text-[15px] text-white/90">
          {tripWindow && (
        ...
        <p className="mt-5 max-w-[440px] text-[13.5px] text-white/[0.78]">
      ...
      <div className="absolute bottom-[10px] right-10 z-30 hidden h-[300px] w-[720px] md:block">
      ...
      {/* ── Mobile title block ──────────────────────────────────────────── */}
      <div className="atelier-rise absolute inset-x-[18px] top-[calc(env(safe-area-inset-top,0px)+88px)] z-[8] md:hidden">
        <div className="mb-3">
          <StatusChip label={status} />
        </div>
        <h1 className="font-display text-[33px] font-semibold leading-[1.06] tracking-[-0.02em] [text-shadow:0_2px_24px_rgba(0,0,0,0.34)]">
          {tripName}
        </h1>
        {tripWindow && (
      ...
      {/* ── Mobile deck: swipe rail + dots ──────────────────────────────── */}
      <div className="absolute inset-x-0 bottom-[calc(env(safe-area-inset-bottom,0px)+24px)] z-[9] md:hidden">
        <div className="flex items-end justify-between px-[18px] pb-3">
          ...
          <span className="font-display text-[15px] italic text-white">
            {activeLoc.name}, {activeLoc.country}
          </span>
        </div>
        <div
...
function CarouselCard({
  ...
  const ty = baseTy - (hover && !isActive ? 12 : 0);
  const rot = hover && !isActive ? 0 : d * 3.5;
  ...
  return (
    <div
      ...
      className="absolute bottom-0 left-1/2 -ml-[95px] h-[250px] w-[190px] overflow-hidden rounded-2xl border-[3px] border-white"
      style={{
        ...
        transition: `transform 0.55s ${EASE}, opacity 0.55s, box-shadow 0.35s`,
      }}
    >
      {isActive ? (
        <Link
...
function MobileCard({
  ...
  return (
    <div
      className="relative h-[178px] w-[150px] flex-shrink-0 overflow-hidden rounded-2xl border-[2.5px] [scroll-snap-align:center]"
      style={{
        ...
        transition: `transform 0.42s ${EASE}, opacity 0.42s, box-shadow 0.42s, border-color 0.42s`,
      }}
    >
      {isActive ? (
        <Link
          ...
          aria-label={`Open ${loc.name}, ${loc.country}`}
        >
          <CardFace loc={loc} active sizes="150px" showArrow />
        </Link>
      ) : (
        <button
          type="button"
        ...
        >
          <CardFace loc={loc} active={false} sizes="150px" showArrow={false} />
        </button>
      )}
    </div>
  );
